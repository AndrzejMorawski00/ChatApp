    ...
    return (
        <ReusableModal open={open} onOpenChange={(newOpen) => setOpen(newOpen)}>
            <ReusableModal.Button asChild>
                <button
                    className="p-1 text-xl duration-300 transform text-textColor hover:scale-105"
            ...
            <ReusableModal.Content title={buttonText}>
                <ChatAction
                    chatID={chatId}
                    chatActionName={chatActionName}
                    onOpenChange={(newValue: boolean) => setOpen(newValue)}
                />
