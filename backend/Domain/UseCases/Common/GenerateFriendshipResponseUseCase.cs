...
namespace Domain.UseCases.Common
{
    ...
    {
        public required Dictionary<string, List<FriendshipModel>> FriendshipResponseModel { get; set; }
    }
    ...
    public class GenerateFriendshipResponseHandler : IRequestHandler<GenerateFriendshipResponseParameters, GenerateFriendshipResponseResults>
    {
        ...
        public async Task<GenerateFriendshipResponseResults> Handle(GenerateFriendshipResponseParameters request, CancellationToken cancellationToken)
        {
            ...
            var relatedUsersQuery = _dbContext.Friends
                .Where(f => f.SenderID == user.ID || f.ReceiverID == user.ID);

            var friendshipModels = await relatedUsersQuery
                .Include(f => f.Sender)
                .Include(f => f.Receiver)
                .Select(f => new FriendshipModel
                {
                    ID = f.ID,
                    SenderID = f.SenderID,
            ...
            var userResponse = await _mediator
                .Send(new DivideFriendshipsResponseParameters
                {
                    Friendships = friendshipModels
                });
