...
namespace Domain.UseCases.Common
{
    ...
    public class GetUserListResponseHandler : IRequestHandler<GetUserListResponseParameters, GetUserListResponseResults>
    {
        ...
        {
            var relatedUsers = await _dbContext.Friends
                .Where(f => f.SenderID == request.User.ID || f.ReceiverID == request.User.ID)
                .Select(f => f.SenderID == request.User.ID ? f.ReceiverID : f.SenderID)
                .ToListAsync();

            var unrealtedUsersQuery = _dbContext.UserData
                .Where(u => !relatedUsers
                .Contains(u.ID) && u.ID != request.User.ID);
            ...
            var unrelatedUsers = await unrealtedUsersQuery
            ...
            return new GetUserListResponseResults
            {
